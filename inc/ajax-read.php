...
class P2Ajax_Read {
	...
	static function tag_search() {
		...
		$tags = array();
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT name, count FROM $wpdb->term_taxonomy AS tt INNER JOIN $wpdb->terms AS t ON tt.term_id = t.term_id WHERE tt.taxonomy = 'post_tag' AND t.name LIKE %s ORDER BY count DESC", '%' . $wpdb->esc_like( $term ) . '%' ) );
