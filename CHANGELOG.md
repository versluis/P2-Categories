...
- Category list limited to the 8 most-used categories (ordered by post count descending); if no pill is selected the post falls back to the WordPress default category
- Full dark mode support via `html.dark-mode #cat-types` overrides matching the dark-mode `#post-types` palette
- Fixed missing `esc_attr()` / `esc_html()` on category slug and name output; categories are managed via the WordPress admin interface
